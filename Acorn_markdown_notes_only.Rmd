...
Also note, it would be better to import data directly from the relevant SQL server, which is quite straight forward in R.
...
The shape file is loaded as a 'SpatialPolygonsDataFrame'. This is a type of data frame that R plots as boundaries on a map. i.e. it is not the Acorn lsoa data CSV file iteself that is plotted. The following steps explain the steps required to plot the Acorn data...

## Shiny App Development

The final product is developed as a Shiny App. This allows for an interactive version of the map to be rendered. Much of what i have learnt with regards to developing Shint Apps can be found in the R Studio tutorial here:
...
There are essentiall two components to the app, a 'ui', which dictates with layout of the app, and a 'server' element, which provides the code for the app to work.
...
If you change the imported files, make sure to change all refernces to the file names to reflect the new file names. In addition, make sure the correct column names are references.
